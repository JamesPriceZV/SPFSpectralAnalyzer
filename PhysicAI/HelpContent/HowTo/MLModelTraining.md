...
Thirty domain-specific physics-informed neural networks that encode spectral physics (Beer-Lambert law, Bragg diffraction, Saha-Boltzmann plasma model, Kramers-Kronig relations, Kohn-Sham DFT, Brus equation, etc.) directly into the model architecture. Modalities span UV-Vis, FTIR, NIR, Raman, Mass Spec (EI and MS/MS), NMR (1H and 13C), Fluorescence, XRD, XPS, EELS, Atomic Emission, LIBS, GC/HPLC Retention, HITRAN, Atmospheric UV/Vis, USGS Reflectance, Optical Constants, SAXS/SANS, Circular Dichroism, Microwave/Rotational, TGA, THz, DFT Quantum Chemistry, Mössbauer, Quantum Dot Photoluminescence, Auger Electron (AES), and Neutron Diffraction spectroscopy.

See <doc:PINNTrainingData> for the complete reference data pipeline covering all 30 modalities.
...
- **Storage:** Compiled models are stored in `Application Support/com.zincoverde.PhysicAI/Models/` and persist across launches.
