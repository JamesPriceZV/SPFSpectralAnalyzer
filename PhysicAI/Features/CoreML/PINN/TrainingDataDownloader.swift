...
/// `~/Library/Application Support/com.zincoverde.PhysicAI/TrainingData/{domain_name}/Downloads/`
///
...
final class TrainingDataDownloader {
    ...
    /// Per-domain download status. Each domain tracks its own progress independently.
    var domainStatus: [PINNDomain: DownloadStatus] = [:]

    /// Per-domain, per-source download status.
    var domainSourceStatuses: [PINNDomain: [String: SourceStatus]] = [:]

    /// Convenience: overall status (true if ANY domain is actively downloading).
    var isAnyDomainActive: Bool {
        domainStatus.values.contains { $0.isActive }
    }

    /// Returns the download status for a specific domain.
    func status(for domain: PINNDomain) -> DownloadStatus {
        domainStatus[domain] ?? .idle
    }

    /// Returns the source statuses for a specific domain.
    func sourceStatuses(for domain: PINNDomain) -> [String: SourceStatus] {
        domainSourceStatuses[domain] ?? [:]
    }

    /// Returns the source status for a specific source in a specific domain.
    func sourceStatus(for sourceName: String, in domain: PINNDomain) -> SourceStatus? {
        domainSourceStatuses[domain]?[sourceName]
    }
    ...
    /// Sets source status scoped to a specific domain.
    private func setSourceStatus(_ status: SourceStatus, for sourceName: String, in domain: PINNDomain) {
        if domainSourceStatuses[domain] == nil {
            domainSourceStatuses[domain] = [:]
        }
    ...
    func downloadSource(_ source: PINNDomain.TrainingDataSource, for domain: PINNDomain) async {
        guard let url = source.url else {
            setSourceStatus(.failed("No URL"), for: source.name, in: domain)
            return
        }
        ...
        setSourceStatus(.downloading(bytesDownloaded: 0, totalBytes: nil), for: source.name, in: domain)
        domainStatus[domain] = .downloading(source: source.name, progress: 0)
        ...
        do {
            ...
            guard let httpResponse = response as? HTTPURLResponse,
                  (200...299).contains(httpResponse.statusCode) else {
                let code = (response as? HTTPURLResponse)?.statusCode ?? 0
                setSourceStatus(.failed("HTTP \(code)"), for: sourceName, in: domain)
                return
            }
            ...
            if downloadedSize == 0 {
                setSourceStatus(.failed("Empty response (0 bytes)"), for: sourceName, in: domain)
                return
            }
            ...
            if let format = detectedFormat {
                ...
                if format == .zip {
                    ...
                    setSourceStatus(.completed(fileSize: fileSize), for: sourceName, in: domain)
                    #else
                    setSourceStatus(.failed("ZIP extraction requires macOS"), for: sourceName, in: domain)
                    #endif
                    return
                }
            ...
            setSourceStatus(.failed(error.localizedDescription), for: sourceName, in: domain)
            Instrumentation.log(
    ...
    /// Browser-like User-Agent for servers (NIST, etc.) that block programmatic access.
    /// Safari on macOS — matches what users see when clicking links in-app.
    ...
    private func downloadWithProgress(url: URL, sourceName: String, domain: PINNDomain) async throws -> (URL, URLResponse) {
        let isSourceForge = url.host?.contains("sourceforge.net") == true
        let config = URLSessionConfiguration.default
        config.timeoutIntervalForRequest = isSourceForge ? 300 : 120
        config.timeoutIntervalForResource = isSourceForge ? 3600 : 1800
        config.httpAdditionalHeaders = [
        ...
        delegate.onProgress = { [weak self] bytesWritten, totalBytes in
            Task { @MainActor [weak self] in
                self?.setSourceStatus(
                    .downloading(bytesDownloaded: bytesWritten, totalBytes: totalBytes > 0 ? totalBytes : nil),
                    for: name, in: dom
                )
            }
        }
    ...
    private func downloadFigshareArticle(articleID: String, source: PINNDomain.TrainingDataSource, domain: PINNDomain) async {
        let sourceName = source.name
        setSourceStatus(.downloading(bytesDownloaded: 0, totalBytes: nil), for: sourceName, in: domain)
        let destDir = Self.downloadDirectory(for: domain)

        do {
            let files = try await Self.resolveFigshareFiles(articleID: articleID)
            guard !files.isEmpty else {
                setSourceStatus(.failed("No files"), for: sourceName, in: domain)
                return
            }
    ...
    func downloadAllSources(for domain: PINNDomain) async {
        ...
        let domainSources = sourceStatuses(for: domain)
        let pending = sources.filter { !(domainSources[$0.name]?.isCompleted ?? false) }
        guard !pending.isEmpty else {
            let files = Self.downloadedFiles(for: domain)
            domainStatus[domain] = .completed(fileCount: files.count)
            return
        }

        domainStatus[domain] = .downloading(source: "0/\(pending.count) sources", progress: 0)
        ...
        await withTaskGroup(of: Void.self) { group in
            ...
            for source in pending {
                ...
                if launched >= Self.maxConcurrentDownloads {
                    ...
                    completed += 1
                    await MainActor.run {
                        self.domainStatus[domain] = .downloading(
            ...
            for await _ in group {
                completed += 1
                domainStatus[domain] = .downloading(
                    source: "\(completed)/\(total) sources",
                    progress: Double(completed) / Double(total)
                )
            }
    ...
    /// Each domain downloads independently so multiple domains can progress in parallel.
    func downloadAllDomains() async {
        ...
        await withTaskGroup(of: Void.self) { group in
            for domain in domains {
                ...
                let capturedDomain = domain
                group.addTask {
                    await downloader.downloadAllSources(for: capturedDomain)
                }
            }
        }
    }
