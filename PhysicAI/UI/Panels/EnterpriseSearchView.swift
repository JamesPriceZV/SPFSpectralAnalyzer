...
struct EnterpriseSearchView: View {
    ...
    var body: some View {
        VStack(spacing: 0) {
            ...
            .tint(activeTab == .teams ? Color(red: 0.45, green: 0.28, blue: 0.76) : .accentColor) // Teams purple
                    ...
                    VStack(alignment: .leading, spacing: 0) {
                        ...
                        if viewModel.isLoading {
                            ...
                            ContentUnavailableView(
                                ...
                                description: Text("Enter a query to search your enterprise content.")
                            )
                        } else {
                            List(viewModel.citations, selection: $selectedCitationID) { citation in
                                iPadResultRow(citation)
                                    .tag(citation.id)
                            }
    ...
    /// iPad-specific header with full-width layout and Microsoft-inspired styling.
    private var iPadHeaderSection: some View {
        HStack(alignment: .center, spacing: 16) {
            ...
            HStack(spacing: 10) {
                ...
                VStack(alignment: .leading, spacing: 2) {
                    Text("Microsoft 365 Enterprise")
                        .font(.title3.bold())
                    HStack(spacing: 6) {
                        Circle()
                            .fill(viewModel.authManager.isSignedIn ? .green : .secondary)
                            .frame(width: 7, height: 7)
                        Text(viewModel.authManager.isSignedIn
                             ? (viewModel.authManager.currentUsername() ?? "Signed In")
                             : "Not signed in")
                            .font(.caption)
            ...
            if viewModel.authManager.isSignedIn {
                ...
                Button {
                    ...
                    Label("Sign In to M365", systemImage: "person.badge.key")
                        .font(.subheadline)
                }
                .controlSize(.regular)
                #if compiler(>=6.2)
    ...
    /// Compact result row for the iPad sidebar list.
    private func iPadResultRow(_ citation: GroundingCitation) -> some View {
    ...
    private func enterpriseResultCard(_ citation: GroundingCitation) -> some View {
        VStack(alignment: .leading, spacing: 8) {
            ...
            HStack(spacing: 12) {
                ...
                Button {
                    UIPasteboard.general.string = citation.extractText
                } label: {
                    Label("Copy Extract", systemImage: "doc.on.doc")
                        .font(.caption)
                }
