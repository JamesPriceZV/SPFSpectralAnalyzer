    ...
    var summaryStrip: some View {
        ...
        return glassGroup(spacing: 12) {
            HStack(spacing: 10) {
                ...
                if selectionCount == 1, let mlPrediction = analysis.coreMLPrediction {
                    metricChip(
                        title: "SPF (ML)",
                        value: mlPrediction.formatted,
                        status: mlPrediction.spfEstimate >= 30 ? .pass : (mlPrediction.spfEstimate >= 15 ? .warn : .fail)
                    )
                }
        ...
        return VStack(alignment: .leading, spacing: 12) {
            HStack(spacing: 6) {
                Text("Dataset Dashboard")
                    .font(.headline)
                HelpButton("Dataset Dashboard", message: "This dashboard summarizes the **sunscreen performance** of all loaded spectra.\n\n**Compliance** \u{2014} Percentage of samples meeting SPF \u{2265}30.\n\n**UVA/UVB Ratio** \u{2014} Measures the balance of UVA (320\u{2013}400 nm) vs UVB (290\u{2013}320 nm) protection. The COLIPA standard requires \u{2265}0.33 for a product to claim 'broad-spectrum' protection.\n\n**Critical Wavelength (\u{03bb}c)** \u{2014} The wavelength where 90% of total UV absorbance is reached. Must be \u{2265}370 nm for broad-spectrum classification.\n\n**Photo-stability** \u{2014} How well the sunscreen maintains its SPF after UV exposure (incubation). <10% loss is excellent; >20% is a concern.")
                Spacer()
            ...
            VStack(alignment: .leading, spacing: 6) {
                HStack(spacing: 4) {
                    Text("Batch UVA/UVB Heatmap")
                        .font(.caption)
                        .foregroundColor(.secondary)
                    HelpButton("UVA/UVB Heatmap", message: "This heatmap plots every spectrum\u{2019}s **UVA/UVB ratio** (x-axis) against its **critical wavelength** (y-axis).\n\nThe **orange dashed line** at 0.33 marks the COLIPA minimum UVA/UVB ratio for broad-spectrum labelling.\n\nThe **green dashed line** at 370 nm marks the FDA/ISO critical wavelength threshold for broad-spectrum classification.\n\nSpectra in the **upper-right quadrant** pass both requirements. Darker cells indicate more samples in that bin.")
                }
