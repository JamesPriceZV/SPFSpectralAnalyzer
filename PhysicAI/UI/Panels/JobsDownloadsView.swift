...
struct JobsDownloadsView: View {
    ...
    private var headerSection: some View {
        VStack(alignment: .leading, spacing: 4) {
            ...
            HStack(spacing: 12) {
                ...
                .buttonStyle(.borderedProminent)
                .controlSize(.small)
                .disabled(downloader.isAnyDomainActive)
    ...
    private var downloadSection: some View {
        VStack(alignment: .leading, spacing: 12) {
            ...
            let activeDomains = PINNDomain.allCases.filter { downloader.status(for: $0).isActive }
            let completedDomains = PINNDomain.allCases.filter {
                if case .completed = downloader.status(for: $0) { return true }
                return false
            }

            if activeDomains.isEmpty && completedDomains.isEmpty {
                HStack(spacing: 8) {
                ...
                // Show per-domain download status
                ForEach(PINNDomain.allCases.filter { downloader.status(for: $0) != .idle }) { domain in
                    VStack(alignment: .leading, spacing: 6) {
                        // Domain header with status
                        switch downloader.status(for: domain) {
                        case .downloading(let source, let progress):
                            downloadProgressRow(
                                name: "\(domain.displayName): \(source)",
                                progress: progress,
                        ...
                        case .staging(let source):
                            HStack(spacing: 8) {
                                ProgressView()
                                    .controlSize(.small)
                                Text("\(domain.displayName): Staging \(source)")
                                    .font(.subheadline)
                            }
                        case .completed(let count):
                            HStack(spacing: 8) {
                                Image(systemName: "checkmark.circle.fill")
                                    .foregroundStyle(.green)
                                Text("\(domain.displayName): \(count) files downloaded")
                                    .font(.subheadline)
                            }
                        case .failed(let msg):
                            HStack(spacing: 8) {
                        ...
                        // Per-source statuses for this domain
                        let domainSources = downloader.sourceStatuses(for: domain)
                        if !domainSources.isEmpty {
                            VStack(alignment: .leading, spacing: 4) {
                                ForEach(
                                    domainSources.sorted(by: { $0.key < $1.key }),
                                    id: \.key
    ...
    private func domainCard(_ domain: PINNDomain) -> some View {
        ...
        return VStack(alignment: .leading, spacing: 6) {
            ...
            HStack(spacing: 8) {
                ...
                .font(.caption)
                .controlSize(.mini)
                .disabled(downloader.status(for: domain).isActive)
