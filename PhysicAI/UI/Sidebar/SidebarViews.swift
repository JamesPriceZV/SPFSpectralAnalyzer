...
extension ContentView {
    ...
    func unloadDataset(id datasetID: UUID) {
        let removed = analysis.unloadSpectra(forDatasetID: datasetID)
    ...
    private func spectrumRowMenuItems(for index: Int, spectrum: ShimadzuSpectrum) -> some View {
        ...
        Button {
            scheduleEventType = .incubation(sampleName: spectrum.name)
        } label: {
            Label("Schedule Re-test", systemImage: "calendar.badge.plus")
        }
