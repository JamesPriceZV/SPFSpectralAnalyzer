...
final class DatasetViewModel {
    ...
    /// Called after spectra are loaded so the view can switch to .analyze mode.
    ...
    func datasetNamePreview(_ datasets: [StoredDataset]) -> String {
        ...
        let names = datasets.map { dataset -> String in
            if let record = searchableRecordCache[dataset.id] ?? archivedSearchableRecordCache[dataset.id] {
                return record.fileName
            }
            // Fall back to UUID string — lowest-risk model access (id is a simple stored UUID)
    ...
    func loadSpectra(from urls: [URL], append: Bool) async {
        ...
        await MainActor.run {
            ...
            if append {
                analysis.selectedSpectrumIndices = analysis.selectedSpectrumIndices.filter { $0 >= 0 && $0 < analysis.spectra.count }
                if wasEmpty, !analysis.spectra.isEmpty {
                    analysis.selectedSpectrumIndex = 0
                    analysis.selectedSpectrumIndices = [0]
                } else {
