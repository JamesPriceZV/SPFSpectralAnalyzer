...
final class MLTrainingService {
    ...
    private func loadPersistedResult() {
        guard FileManager.default.fileExists(atPath: Self.trainingResultURL.path) else { return }
        do {
            let data = try Data(contentsOf: Self.trainingResultURL)
            lastResult = try JSONDecoder().decode(MLTrainingResult.self, from: data)
        } catch {
            Instrumentation.log("Failed to load ML training result", area: .processing, level: .warning,
    ...
    private func persistResult(_ result: MLTrainingResult) {
        do {
            ...
            Instrumentation.log("Failed to persist ML training result", area: .processing, level: .warning,
                                details: "error=\(error.localizedDescription)")
        }
    }
    ...
    func updateAvailableCount(modelContext: ModelContext) {
        ...
        do {
            ...
            for dataset in datasets {
                ...
                let validSpectra = (try? modelContext.fetch(FetchDescriptor<StoredSpectrum>(predicate: spectraPredicate))) ?? []
                count += validSpectra.count
            }
    ...
    func train(modelContext: ModelContext) async {
        ...
        Instrumentation.log("ML training started", area: .processing, level: .info)
        ...
        status = .complete
        Instrumentation.log("ML training complete", area: .processing, level: .info,
    ...
    private func buildTrainingData(modelContext: ModelContext) async throws -> TrainingDataBuildResult {
        ...
        for dataset in datasets {
            ...
            for spectrum in spectra {
                ...
                let isPost = FilenameMetadataParser.parse(filename: spectrum.name).isPostIrradiation
