...
enum PDFReportRenderer {
    ...
    // Colors
    static let accentColor = NSColor(red: 0.2, green: 0.4, blue: 0.7, alpha: 1.0)
    static let lightGray = NSColor.gray.withAlphaComponent(0.3)
    ...
    static let headingFont = NSFont.systemFont(ofSize: 14, weight: .semibold)
    static let bodyFont = NSFont.systemFont(ofSize: 10, weight: .regular)
    static let bodySemibold = NSFont.systemFont(ofSize: 10, weight: .semibold)
    ...
    static func render(data: PDFReportData) -> Data {
        ...
        if hasAIContent {
            ...
            if !data.risks.isEmpty {
                cursor.ensureSpace(30)
                let riskColor = NSColor(red: 0.8, green: 0.2, blue: 0.2, alpha: 1.0)
            ...
            if !data.actions.isEmpty {
                cursor.ensureSpace(30)
                let actionColor = NSColor(red: 0.2, green: 0.6, blue: 0.3, alpha: 1.0)
    ...
    private static func drawSPFCallout(cursor: inout PDFCursor, estimation: SPFEstimationResult) {
        ...
        case .fullColipa: bgColor = NSColor(red: 0.85, green: 0.95, blue: 0.85, alpha: 1.0)
        case .calibrated: bgColor = NSColor(red: 0.85, green: 0.9, blue: 1.0, alpha: 1.0)
        case .adjusted:   bgColor = NSColor(red: 1.0, green: 0.93, blue: 0.82, alpha: 1.0)
        }
        ...
        let spfFont = NSFont.systemFont(ofSize: 18, weight: .bold)
    ...
    private static func drawBulletSection(cursor: inout PDFCursor, title: String, items: [String], bulletColor: NSColor) {
    ...
    private static func drawRecommendationsTable(cursor: inout PDFCursor, recommendations: [ReportRecommendation]) {
        for (index, rec) in recommendations.enumerated() {
            ...
            if index < recommendations.count - 1 {
                cursor.ctx.setStrokeColor(NSColor.gray.withAlphaComponent(0.15).cgColor)
                cursor.ctx.setLineWidth(0.5)
    ...
    private static func drawMetricsTable(cursor: inout PDFCursor, rows: [ReportMetricRow]) {
        ...
        cursor.ctx.setFillColor(NSColor(white: 0.95, alpha: 1.0).cgColor)
        ...
        for (index, row) in rows.enumerated() {
            ...
            if index % 2 == 0 {
                cursor.ctx.setFillColor(NSColor(white: 0.98, alpha: 1.0).cgColor)
                cursor.ctx.fill(CGRect(x: margin, y: cursor.y - rowHeight, width: contentWidth, height: rowHeight))
            }
    ...
    private static func drawSpectraChart(cursor: inout PDFCursor, series: [ReportSpectrumSeries]) {
        ...
        // Axes
        cursor.ctx.setStrokeColor(NSColor.gray.cgColor)
        cursor.ctx.setLineWidth(0.5)
        ...
        // Axis labels
        let axisLabelAttrs: [NSAttributedString.Key: Any] = [.font: NSFont.systemFont(ofSize: 8), .foregroundColor: NSColor.gray]
        ...
        for i in 0...3 {
            ...
            let labelSize = cursor.textSize(label, font: NSFont.systemFont(ofSize: 8))
            cursor.drawTextAt(label, x: chartRect.minX - labelSize.width - 4, y: y - 5, attrs: axisLabelAttrs)
        }

        // Axis titles
        let axisTitleAttrs: [NSAttributedString.Key: Any] = [.font: NSFont.systemFont(ofSize: 9, weight: .semibold), .foregroundColor: NSColor.darkGray]
        let xAxisTitle = "Wavelength (nm)"
        let xTitleSize = cursor.textSize(xAxisTitle, font: NSFont.systemFont(ofSize: 9, weight: .semibold))
        ...
        let ytAttrs: [NSAttributedString.Key: Any] = [.font: NSFont.systemFont(ofSize: 9, weight: .semibold) as CTFont, .foregroundColor: NSColor.darkGray.cgColor]
        ...
        let palette: [NSColor] = [
            ...
            NSColor(red: 0.8, green: 0.5, blue: 0.1, alpha: 1.0),
...
struct PDFCursor {
    ...
    mutating func drawText(_ text: String, font: NSFont, color: NSColor) {
    ...
    func drawTextAt(_ text: String, x: CGFloat, y: CGFloat, attrs: [NSAttributedString.Key: Any]) {
        ...
        for (key, value) in attrs {
            if key == .font, let nsFont = value as? NSFont {
                ctAttrs[key] = nsFont as CTFont
            } else if key == .foregroundColor, let nsColor = value as? NSColor {
    ...
    func truncatedText(_ text: String, font: NSFont, maxWidth: CGFloat) -> String {
    ...
    mutating func drawWrappedText(_ text: String, font: NSFont, color: NSColor, indent: CGFloat = 0) {
