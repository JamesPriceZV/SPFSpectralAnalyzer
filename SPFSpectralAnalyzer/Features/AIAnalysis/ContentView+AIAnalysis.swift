...
extension ContentView {
        ...
        Task {
            ...
            do {
                ...
                await MainActor.run {
                    ...
                    // Surface fallback notice if a requested provider was unavailable
                    if let notice = aiVM.providerManager.fallbackNotice {
                        aiVM.fallbackNotice = notice
                    }

                    // Show ensemble comparison if multiple providers returned results
    ...
    /// Note: the routing JSON is always decoded if non-empty — the Advanced Routing
    /// toggle only controls Settings UI visibility, not whether routing is applied.
    ...
    func buildEnsembleConfig() -> EnsembleConfig {
        ...
        if !aiEnsembleProvidersJSON.isEmpty,
           ...
           let rawValues = try? JSONDecoder().decode([String].self, from: data) {
            providers = Set(rawValues.compactMap { AIProviderID(rawValue: $0) })
        }
        // Fall back to first 2+ providers from priority order that have credentials
        if providers.count < 2 {
            let credentials = buildProviderCredentials()
            let available = decodedPriorityOrder.filter {
                aiVM.providerManager.makeProvider(for: $0, credentials: credentials) != nil
            }
            if available.count >= 2 {
                providers = Set(available)
            }
        }
        guard providers.count >= 2 else { return EnsembleConfig() }
        return EnsembleConfig(isEnabled: true, selectedProviders: providers, arbitrationEnabled: aiEnsembleArbitrationEnabled)
    }
    ...
    private func buildMLPredictionPayload(spectra: [ShimadzuSpectrum], yAxisMode: SpectralYAxisMode) -> AIMLPredictionPayload? {
        ...
        // Determine post-irradiation: use dataset manual override if set, else auto-detect from spectrum name
        let isPostIrrad = dataset?.isPostIrradiation
