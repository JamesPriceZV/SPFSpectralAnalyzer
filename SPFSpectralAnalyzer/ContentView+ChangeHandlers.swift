...
extension ContentView {
    ...
    func applyImporters<V: View>(_ view: V) -> some View {
        ...
        #if os(macOS)
        // SPC file importer is now attached directly to importPanel
        // to avoid multiple .fileImporter modifiers on the same view.
        view
            .fileImporter(
    ...
    func applyAlertsAndSheets<V: View>(_ view: V) -> some View {
        view
            ...
            .sheet(isPresented: $showBatchComparePopout) {
                BatchCompareWindowView()
                    .environment(aiVM.m365AuthManager)
                    #if os(iOS)
            ...
            .confirmationDialog(datasets.directDeleteConfirmationTitle(), isPresented: $datasets.showDirectDeleteConfirmation, titleVisibility: .visible) {
                Button("Delete Permanently", role: .destructive) {
                    datasets.permanentlyDeleteActiveDatasets(storedDatasets: storedDatasets)
                }
                Button("Cancel", role: .cancel) {
                    datasets.pendingDirectDeleteIDs.removeAll()
                }
            } message: {
