...
struct iOSAnalysisView: View {
    ...
    private var iPadLayout: some View {
        NavigationSplitView {
            datasetSidebar
                ...
                .navigationSplitViewColumnWidth(min: 300, ideal: 420, max: 420)
    ...
    private var datasetSidebar: some View {
        List {
            ...
            // MARK: Stored Datasets — Prototypes Only (Load & Append)
            if showStoredDatasets {
                let prototypeDatasets = storedDatasets.filter { dataset in
                    let cache = datasets.searchableRecordCache[dataset.id]
                    return (cache?.isPrototype ?? false) && !(cache?.isArchived ?? false)
                }
                Section("Prototype Samples — \(prototypeDatasets.count)") {
                    if prototypeDatasets.isEmpty {
                        Text("No prototype samples available.\nTag datasets as Prototypes in the Data tab.")
                            .font(.caption)
                            .foregroundColor(.secondary)
                    } else {
                        ForEach(prototypeDatasets) { dataset in
    ...
    private func dashboardCard(title: String, value: String, detail: String, statusColor: Color = .secondary) -> some View {
        ...
        .padding(12)
        .glassEffect(.clear.tint(statusColor.opacity(0.15)), in: .rect(cornerRadius: 12))
            ...
            HStack {
                if aiVM.isRunning {
                    HStack(spacing: 6) {
                        ProgressView()
                            .controlSize(.small)
                        Text(aiVM.providerManager.activeProviderName ?? "Resolving provider\u{2026}")
                            .font(.caption)
                            .foregroundColor(.secondary)
                    }
                } else {
