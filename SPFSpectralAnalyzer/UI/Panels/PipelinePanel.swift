...
extension ContentView {
    ...
    var batchComparePanel: some View {
        VStack(alignment: .leading, spacing: 8) {
            HStack {
                ...
                .buttonStyle(.plain)
                .foregroundColor(.secondary)
                .help("Open in resizable window")
                .disabled(batchCompareRows.isEmpty)
    ...
    func popOutBatchCompare() {
        BatchCompareWindowModel.shared.update(
            rows: batchCompareRows,
            sourceLabel: batchCompareSourceLabel
        )
        #if os(macOS)
        openWindow(id: "batch-compare")
        #else
        showBatchComparePopout = true
        #endif
    }
