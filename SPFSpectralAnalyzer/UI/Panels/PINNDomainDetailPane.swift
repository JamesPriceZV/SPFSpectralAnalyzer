    ...
    private func sourceStatusIcon(for source: PINNDomain.TrainingDataSource) -> some View {
        if source.isLicensed {
            ...
            Image(systemName: "person.fill")
                .font(.system(size: 9))
                .foregroundColor(.secondary)
        } else if let sourceStatus = trainingDataDownloader.sourceStatuses[source.name] {
            switch sourceStatus {
            case .completed:
            ...
            VStack(alignment: .leading, spacing: 10) {
                ...
                ForEach(Array(domain.trainingDataSourcesWithURLs.enumerated()), id: \.offset) { _, source in
                    VStack(alignment: .leading, spacing: 3) {
                        ...
                        if let sourceStatus = trainingDataDownloader.sourceStatuses[source.name] {
                ...
                // Download progress
                if case .downloading(let source, _) = trainingDataDownloader.status {
                    HStack(spacing: 6) {
                        ProgressView()
                            .controlSize(.small)
                ...
                } else if case .staging(let source) = trainingDataDownloader.status {
                    HStack(spacing: 6) {
                        ProgressView()
                            .controlSize(.small)
                        Text("Staging \(source)...")
                ...
                } else if case .completed(let count) = trainingDataDownloader.status {
