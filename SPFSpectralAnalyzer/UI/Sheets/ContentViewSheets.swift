...
extension ContentView {
    ...
    var samplePlateTypeSheet: some View {
        VStack(spacing: 16) {
            ...
            if let pendingID = datasets.pendingRoleDatasetID,
               let record = datasets.searchableRecordCache[pendingID] {
            ...
            if datasets.pendingPlateType == .pmma {
                ...
                // HDRS Plate Type — only relevant for PMMA substrates
                VStack(spacing: 8) {
            ...
            // Irradiation State
            VStack(spacing: 8) {
                Text("Irradiation State")
                    .font(.subheadline.bold())
                    .foregroundColor(.secondary)
                    .frame(maxWidth: .infinity, alignment: .center)

                Picker("Irradiation", selection: $datasets.pendingIrradiationOverride) {
                    Text("Auto-Detect").tag(Bool?.none)
                    Text("Pre-Irradiation").tag(Bool?.some(false))
                    Text("Post-Irradiation").tag(Bool?.some(true))
                }
                .labelsHidden()
                #if os(macOS)
                .pickerStyle(.radioGroup)
            ...
            HStack(spacing: 12) {
                ...
                Button("Save") {
                    if let datasetID = datasets.pendingRoleDatasetID {
                        ...
                        // Persist irradiation state override
                        datasets.setIrradiationStatus(
                            datasets.pendingIrradiationOverride,
                            for: datasetID,
                            storedDatasets: storedDatasets
                        )
                        // Always propagate formula card state — including nil (removal)
                        datasets.setFormulaCard(
                            id: datasets.pendingFormulaCardID,
                            for: datasetID,
                            storedDatasets: storedDatasets
                        )
                    }
