            ...
            Section("Python Environment") {
                ...
                VStack(alignment: .leading, spacing: 8) {
                            ...
                            ForEach(domainsWithData, id: \.domain) { item in
                                HStack(spacing: 6) {
                                    ...
                                    Text("\(item.domain.displayName)")
                                        .font(.caption)
                                    Spacer()
                                    Text("\(item.fileCount) file\(item.fileCount == 1 ? "" : "s") · \(TrainingDataDownloader.formattedSize(bytes: item.size))")
                                        .font(.caption2)
                                        .foregroundColor(.secondary)
                                }
                            }
                    ...
                    HStack(spacing: 8) {
                        ...
                        .buttonStyle(.borderedProminent)
                        .controlSize(.small)
                        .disabled(trainingDataDownloader.status.isActive)
                    ...
                    // Download progress
                    if trainingDataDownloader.status.isActive {
                        VStack(alignment: .leading, spacing: 6) {
                            HStack(spacing: 8) {
                                ProgressView()
                                    .controlSize(.small)
                                switch trainingDataDownloader.status {
                            ...
                            if case .downloading(_, let progress) = trainingDataDownloader.status {
                                ProgressView(value: progress)
                                    .progressViewStyle(.linear)
                            }
                        }
                    } else if case .completed(let fileCount) = trainingDataDownloader.status {
                        HStack(spacing: 6) {
                            Image(systemName: "checkmark.circle.fill")
                                .foregroundColor(.green)
                                .font(.caption)
                            Text("Download complete — \(fileCount) file\(fileCount == 1 ? "" : "s") staged")
                                .font(.caption)
                                .foregroundColor(.green)
                        }
                    } else if case .failed(let msg) = trainingDataDownloader.status {
                        HStack(spacing: 6) {
                            Image(systemName: "exclamationmark.triangle.fill")
                                .foregroundColor(.red)
                                .font(.caption)
            ...
            Section("Model Storage") {
                LabeledContent("Models Path") {
                    let modelsPath = FileManager.default.urls(for: .applicationSupportDirectory, in: .userDomainMask).first?
                        .appendingPathComponent("com.zincoverde.SPFSpectralAnalyzer/Models").path ?? "Unknown"
                    Text(modelsPath)
                        .font(.caption2)
                        .monospaced()
                        .lineLimit(1)
                ...
                Button {
                    if let dir = FileManager.default.urls(for: .applicationSupportDirectory, in: .userDomainMask).first?
                        .appendingPathComponent("com.zincoverde.SPFSpectralAnalyzer/Models") {
                        try? FileManager.default.createDirectory(at: dir, withIntermediateDirectories: true)
                        PlatformURLOpener.open(dir)
                    }
                } label: {
